...
// Version 3.4.0 - 改用 Qdrant 1.15.2+ 內建 BM25 sparse vector 做混合搜尋
...
const defaultSettings = {
  ...
  // v3.4.0：BM25 混合搜尋（Qdrant 1.15.2+ server-side inference）
  hybridSearchEnabled: true,
  hybridVectorWeight: 0.7,        // dense 向量權重，BM25 = 1 - 此值
  hybridPrefetchLimit: 20,        // 兩邊各取多少筆給 client-side fusion 用
  bm25ModelName: "Qdrant/bm25",   // Qdrant 內建 BM25 inference 模型
  denseVectorName: "dense",       // 集合內 dense 向量的命名
  sparseVectorName: "bm25",       // 集合內 sparse 向量的命名
}
...
async function collectionExists(collectionName) {
  try {
    ...
    if (response.status === 404) {
      return { exists: false, vectorSize: null, isNamed: false, hasSparse: false }
    }
    ...
    // v3.4.0：先試 named "dense"，再試 unnamed（向後相容判斷）
    let vectorSize = null
    let isNamed = false

    const denseName = settings.denseVectorName || "dense"
    if (vectorsConfig && typeof vectorsConfig === "object" && vectorsConfig[denseName]?.size) {
      vectorSize = vectorsConfig[denseName].size
      isNamed = true
    } else if (vectorsConfig?.size) {
      // 舊格式（unnamed）
      vectorSize = vectorsConfig.size
      isNamed = false
    } else if (data?.result?.vectors?.size) {
      vectorSize = data.result.vectors.size
      isNamed = false
    }

    // 偵測 sparse vector 是否存在
    const sparseConfig = data?.result?.config?.params?.sparse_vectors
    const sparseName = settings.sparseVectorName || "bm25"
    const hasSparse = !!(sparseConfig && typeof sparseConfig === "object" && sparseConfig[sparseName])

    return { exists: true, vectorSize, isNamed, hasSparse }
  } catch (error) {
...
async function createCollection(collectionName, vectorSize) {
  try {
    ...
    // v3.4.0：永遠用 named vectors。即使目前沒啟用混合搜尋也加 sparse 配置，
    // 這樣使用者之後打開混合搜尋時不需重建集合。
    const body = {
      vectors: {
        [denseName]: {
          size: dimensions,
      ...
      sparse_vectors: {
        [sparseName]: {
          modifier: "idf", // BM25 需要 IDF modifier
        },
      },
    }
    ...
    if (response.ok) {
      console.log(`[Qdrant Memory] 已建立集合 ${collectionName}（dense=${denseName}/${dimensions}d + sparse=${sparseName} BM25）`)
      return true
    } else {
      let errMsg = `${response.status} ${response.statusText}`
      try {
        const errData = await response.json()
        if (errData?.status?.error) errMsg += ` - ${errData.status.error}`
      } catch (e) {}
      console.error(`[Qdrant Memory] 建立集合失敗 ${collectionName}：${errMsg}`)
      console.error(`[Qdrant Memory] 提示：BM25 sparse vector 需要 Qdrant 1.15.2 以上版本`)
      return false
    }
  } catch (error) {
...
async function ensureCollection(characterName, vectorSize) {
  ...
  const { exists, vectorSize: existingSize, isNamed, hasSparse } = info
  ...
  if (exists) {
    if (
      ...
      console.error(
        `[Qdrant Memory] 集合 ${collectionName} 維度為 ${existingSize}，但嵌入結果是 ${vectorSize}。請刪除集合後重建。`,
      )
      return false
    }

    // v3.4.0：偵測舊格式（unnamed 或無 sparse）並警告，但不阻擋使用
    if (settings.hybridSearchEnabled) {
      if (!isNamed) {
        console.warn(
          `[Qdrant Memory] 集合 ${collectionName} 使用舊格式（unnamed vector），無法做 BM25 混合搜尋。請從「檢視記憶」刪除後重建。本次仍以純向量搜尋進行。`,
        )
      } else if (!hasSparse) {
        console.warn(
          `[Qdrant Memory] 集合 ${collectionName} 缺少 BM25 sparse 配置。請刪除後重建以啟用混合搜尋。本次仍以純向量搜尋進行。`,
        )
      }
    }
...
// ============================================================================
// v3.4.0：集合格式快取（避免每次 save/search 都重新查 schema）
// ============================================================================

// collectionName → { isNamed, hasSparse }
const collectionFormatCache = new Map()

async function getCollectionFormat(collectionName) {
  if (collectionFormatCache.has(collectionName)) {
    return collectionFormatCache.get(collectionName)
  }
  const info = await collectionExists(collectionName)
  const format = {
    isNamed: !!info.isNamed,
    hasSparse: !!info.hasSparse,
  }
  if (info.exists) {
    collectionFormatCache.set(collectionName, format)
  }
  return format
}

function invalidateCollectionFormatCache(collectionName) {
  collectionFormatCache.delete(collectionName)
}
...
async function chunkExistsInCollection(collectionName, embedding, text, dedupeThreshold) {
  try {
    const format = await getCollectionFormat(collectionName)
    let url, body

    if (format.isNamed) {
      // v3.4.0：用新的 Query API + named dense
      url = `${settings.qdrantUrl}/collections/${encodeCollectionName(collectionName)}/points/query`
      body = {
        query: embedding,
        using: settings.denseVectorName || "dense",
        limit: 1,
      ...
      // 舊格式（unnamed）退路
      url = `${settings.qdrantUrl}/collections/${encodeCollectionName(collectionName)}/points/search`
      body = {
        vector: embedding,
        limit: 1,
        score_threshold: dedupeThreshold,
    ...
    const data = await response.json()
    // /points/query 回 { result: { points: [...] } }；/points/search 回 { result: [...] }
    const results = format.isNamed ? (data?.result?.points || []) : (data?.result || [])
...
// ============================================================================
// v3.4.0：BM25 sparse 搜尋（Qdrant 1.15.2+ server-side inference）與混合融合
// ============================================================================

/**
 * 用 Qdrant 內建 BM25 inference 對指定集合做 sparse 搜尋。
 * Qdrant 會在 server 端把 text 轉成 sparse vector（IDF 由集合維護），回傳真正的 BM25 分數。
 *
 * @param {string} collectionName
 * @param {string} query - 查詢文字
 * @param {number} limit
 * @param {object|null} extraFilter - 額外的過濾條件（共用集合時帶 character）
 * @returns {Promise<Array>} 點位陣列，每筆有 score（BM25）
 */
async function sparseSearchBM25(collectionName, query, limit, extraFilter) {
  ...
  try {
    const body = {
      query: {
        text: query,
        model: settings.bm25ModelName || "Qdrant/bm25",
      },
      using: settings.sparseVectorName || "bm25",
      limit: limit,
      with_payload: true,
    }

    if (extraFilter && extraFilter.must && extraFilter.must.length > 0) {
      body.filter = extraFilter
    }

    const response = await fetch(
      `${settings.qdrantUrl}/collections/${encodeCollectionName(collectionName)}/points/query`,
      {
        method: "POST",
        headers: getQdrantHeaders(),
        body: JSON.stringify(body),
    ...
    if (!response.ok) {
      ...
      console.warn(
        `[Qdrant Memory] BM25 搜尋失敗（${collectionName}）：${response.status} ${errText.substring(0, 300)}`
      )
      console.warn(`[Qdrant Memory] 提示：BM25 inference 需要 Qdrant 1.15.2+`)
      return []
    }
    ...
    if (settings.debugMode) {
      console.warn(`[Qdrant Memory] sparseSearchBM25 error:`, error)
    }
    return []
  }
}
...
/**
 * 用 Qdrant Query API 做 dense 向量搜尋（named "dense"）
 */
async function denseSearch(collectionName, embedding, limit, extraFilter) {
  try {
    const body = {
      query: embedding,
      using: settings.denseVectorName || "dense",
    ...
    if (!response.ok) {
      console.error(`[Qdrant Memory] Dense 搜尋失敗（${collectionName}）：${response.status} ${response.statusText}`)
      return []
    }
    ...
    console.error(`[Qdrant Memory] denseSearch error:`, error)
    return []
  }
}

/**
 * 將分數陣列歸一化到 0-1 區間（min-max）
 */
function normalizeScoreRange(scores) {
...
/**
 * 合併 dense 與 sparse(BM25) 搜尋結果，加權排序。
 * - dense 分數：cosine 相似度，原本 0-1，再做歸一化
 * - BM25 分數：來自 Qdrant 內建計算，無上限，需要歸一化
 * - 兩邊都命中的點位取兩邊分數加權和；只在一邊命中的，另一邊補 0
 *
 * @returns {Array} 合併後結果，按混合分數由高到低排序
 */
function mergeHybridResults(denseResults, sparseResults, denseWeight, sparseWeight) {
  const dScoresNorm = normalizeScoreRange(denseResults.map((r) => r.score || 0))
  const sScoresNorm = normalizeScoreRange(sparseResults.map((r) => r.score || 0))
  ...
  denseResults.forEach((r, i) => {
    merged.set(r.id, {
      result: r,
      dScore: dScoresNorm[i] ?? 0,
      sScore: 0,
      origDScore: r.score || 0,
      origSScore: 0,
    })
  })
  ...
  sparseResults.forEach((r, i) => {
    if (merged.has(r.id)) {
      ...
      entry.sScore = sScoresNorm[i] ?? 0
      entry.origSScore = r.score || 0
    } else {
      merged.set(r.id, {
        result: r,
        dScore: 0,
        sScore: sScoresNorm[i] ?? 0,
        origDScore: 0,
        origSScore: r.score || 0,
      })
    }
  })

  const combined = []
  merged.forEach(({ result, dScore, sScore, origDScore, origSScore }) => {
    const finalScore = dScore * denseWeight + sScore * sparseWeight
    combined.push({
      ...result,
      score: finalScore,
      _origDenseScore: origDScore,
      _origBM25Score: origSScore,
      _hybridD: dScore,
      _hybridS: sScore,
    })
  })
...
async function searchMemories(query, characterName) {
  ...
  try {
    ...
    const extraFilter = filterConditions.length > 0 ? { must: filterConditions } : null

    // v3.4.0：混合搜尋 - 並行發 dense 與 BM25 sparse 兩個請求
    let results = []
    const format = await getCollectionFormat(collectionName)
    const useHybrid = settings.hybridSearchEnabled && format.isNamed && format.hasSparse
    const dWeight = Number.isFinite(settings.hybridVectorWeight) ? settings.hybridVectorWeight : 0.7
    const sWeight = 1 - dWeight
    const prefetchLimit = Math.max(settings.memoryLimit * 2, settings.hybridPrefetchLimit || 20)

    if (useHybrid && sWeight > 0) {
      const [denseRes, sparseRes] = await Promise.all([
        denseSearch(collectionName, embedding, prefetchLimit, extraFilter),
        sparseSearchBM25(collectionName, query, prefetchLimit, extraFilter),
      ])

      if (settings.debugMode) {
        console.log(`[Qdrant Memory] 混合搜尋：dense ${denseRes.length} 筆、BM25 ${sparseRes.length} 筆`)
      }

      results = mergeHybridResults(denseRes, sparseRes, dWeight, sWeight)

      if (settings.scoreThreshold > 0) {
        results = results.filter((r) => r.score >= settings.scoreThreshold)
      }
    } else if (format.isNamed) {
      // 集合是 named 但混合搜尋關閉或 sWeight=0 → 純 dense
      const denseRes = await denseSearch(collectionName, embedding, settings.memoryLimit * 2, extraFilter)
      results = denseRes.filter((r) => r.score >= settings.scoreThreshold)
    } else {
      // 舊格式（unnamed）集合 → 走舊的 /points/search
      const searchPayload = {
        vector: embedding,
      ...
      if (extraFilter) searchPayload.filter = extraFilter
...
async function saveChunkToQdrant(chunk, participants) {
  ...
  try {
    ...
    const savePromises = participants.map(async (characterName) => {
      ...
      // v3.4.0：依集合格式組 vector 結構
      const format = await getCollectionFormat(collectionName)
      const denseName = settings.denseVectorName || "dense"
      const sparseName = settings.sparseVectorName || "bm25"

      let pointVector
      if (format.isNamed && format.hasSparse && settings.hybridSearchEnabled) {
        // 新格式：dense + sparse（BM25 由 server-side inference）
        pointVector = {
          [denseName]: embedding,
          [sparseName]: {
            text: chunk.text,
        ...
        // 新格式但沒 sparse 或關閉混合：只寫 dense
        pointVector = {
          [denseName]: embedding,
        }
      } else {
        // 舊格式（unnamed）
        pointVector = embedding
      }
      ...
      if (!response.ok) {
        ...
        console.error(
          `[Qdrant Memory] Failed to save chunk to ${characterName}: ${errMsg}`,
        )
        return false
      }
...
<div id="qdrant_local_model_group" style="margin: 10px 0; display: none;">
            ...
            <div style="margin: 10px 0; padding: 10px; background: #e3f2fd; border-left: 4px solid #2196f3; border-radius: 4px;">
                <p style="margin: 0 0 6px 0; font-size: 0.9em;"><strong>🔍 混合搜尋（dense 向量 + BM25）</strong></p>
                <p style="margin: 0 0 8px 0; font-size: 0.85em; color: #555; line-height: 1.5;">
                    同時用語意向量搜尋與 <strong>BM25 關鍵字搜尋</strong>，由 Qdrant server 端內建 BM25 inference（IDF 自動維護），結果在 client 端依權重加權合併。<br>
                    <strong>需要 Qdrant 1.15.2 以上版本</strong>。新建集合會自動配置 dense + sparse 雙向量結構；舊集合（v3.3 以前）需要刪除後重建才能享有混合搜尋。
                </p>
                <label style="display: flex; align-items: center; gap: 10px; margin-top: 6px;">
                    <input type="checkbox" id="qdrant_hybrid_enabled" ${settings.hybridSearchEnabled ? "checked" : ""} />
                    <strong>啟用混合搜尋</strong>
                </label>
                <div style="margin-top: 10px;">
                    <label style="font-size: 0.9em;">
                        <strong>向量 vs. BM25 權重比例：</strong>
                        向量 <span id="hybrid_v_display">${(settings.hybridVectorWeight * 100).toFixed(0)}%</span>
                        ／BM25 <span id="hybrid_f_display">${((1 - settings.hybridVectorWeight) * 100).toFixed(0)}%</span>
                    </label>
                    <input type="range" id="qdrant_hybrid_weight" min="0" max="1" step="0.05" value="${settings.hybridVectorWeight}"
                           style="width: 100%; margin-top: 5px;" />
                    <small style="color: #666;">向左：偏好關鍵字命中；向右：偏好語意相似</small>
                </div>
            </div>
  ...
  // v3.4.0：混合搜尋控制
  $("#qdrant_hybrid_enabled").on("change", function () {
    settings.hybridSearchEnabled = $(this).is(":checked")
    // BM25 結構在集合建立時就決定，這裡不需要額外動作
    // 但要清快取以便重新檢查既有集合的格式
    collectionFormatCache.clear()
  })

  $("#qdrant_hybrid_weight").on("input", function () {
    const v = Number.parseFloat($(this).val())
    settings.hybridVectorWeight = Number.isFinite(v) ? v : 0.7
    $("#hybrid_v_display").text(`${(settings.hybridVectorWeight * 100).toFixed(0)}%`)
    $("#hybrid_f_display").text(`${((1 - settings.hybridVectorWeight) * 100).toFixed(0)}%`)
  })
...
window.jQuery(async () => {
  ...
  console.log("[Qdrant Memory] 擴充功能載入完成（v3.4.0 - Qdrant 1.15.2+ BM25 sparse 混合搜尋）")
})
